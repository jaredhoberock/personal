...
class TypeVariable(object):
    ...
    def __init__(self):
        #print("TypeVariable.__init__(): returning {0}".format(TypeVariable.next_variable_id))
...
def analyse(node, env, non_generic=None):
    ...
    if isinstance(node, Ident):
        #print("analyse(Ident): non_generic: ", non_generic)
        #print("analyse(Ident): calling getType")
        return getType(node.name, env, non_generic)
    elif isinstance(node, Apply):
        #print("analyse(Apply): non_generic:")
        #print(non_generic)
        fun_type = analyse(node.fn, env, non_generic)
        arg_type = analyse(node.arg, env, non_generic)
        #print("analyse(Apply): calling TypeVariable()")
        result_type = TypeVariable()
        reference_type = Function(arg_type, result_type)
        unify(reference_type, fun_type)
        return result_type
    elif isinstance(node, Lambda):
        #print("analyse(Lambda): calling TypeVariable()")
        arg_type = TypeVariable()
        new_env = env.copy()
        new_env[node.v] = arg_type
        new_non_generic = non_generic.copy()
        new_non_generic.add(arg_type)
        #print("analyse(Lambda): new_non_generic: ", new_non_generic)
...
def getType(name, env, non_generic):
    ...
    """
    if name in env:
        #print("getType(): calling fresh on {0}".format(name))
    ...
    def freshrec(tp):
        ...
        if isinstance(p, TypeVariable):
            if isGeneric(p, non_generic):
                #print("{0} is generic".format(p.id))
                if p not in mappings:
                    #print("{0} is not in mappings".format(p.id))
                    mappings[p] = TypeVariable()
                return mappings[p]
            else:
                #print("{0} is not generic".format(p.id))
                return p
...
def definitive(t):
     """Returns the definitive instance of a type.
...
def isGeneric(v, non_generic):
    ...
    """
    return not occursIn(definitive(v), non_generic)
...
def occursInType(v, type2):
    ...
    """
    definitive_v = definitive(v)
    definitive_type2 = definitive(type2)
    if definitive_type2 == definitive_v:
        return True
    elif isinstance(definitive_type2, TypeOperator):
        return occursIn(definitive_v, definitive_type2.types)
    return False
...
def main():
    ...
    return_one = Lambda("x", Ident("1"))
    apply_one  = Lambda("y", Apply(Ident("y"), Ident("1")))
    ...
    examples = [
            ...
            Lambda("g",
             ...
             # fn f => f 5
             Lambda("f", Apply(Ident("f"), Ident("5"))),

             # f = fn x => 1
             # g = fn y => y 1
             # (g f)
             Apply(apply_one, return_one),
    ]
